...
router.post('/search', async (req, res, next) => {
	try {
		if (req.body.searchtype == 'ID телескопа')
			var data = await db.telescopeVisibility(req.body.searchtext)
		else
			var data = await db.objectVisibility(req.body.searchtext)
		res.render('visibility', {
			title : 'Видимость',
			login : req.user,
...
router.get('/:id/delete', [
	async (req, res, next) => {
		try {
			var data = await db.deleteVisibilityByID(req.params.id)
			res.redirect('/visibility')
		}
		catch(err) {
			next(err)
